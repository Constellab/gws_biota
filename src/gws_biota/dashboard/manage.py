...
if 'gws_biota' not in sys.modules:
    biota_lib_path = "/lab/user/bricks/gws_biota/src"
    if not os.path.exists(biota_lib_path):
        biota_lib_path = "/lab/.sys/bricks/gws_biota/src"
        if not os.path.exists(biota_lib_path):
            raise Exception("Cannot find gws_biota brick")
    sys.path.insert(0, biota_lib_path)
