...
class BiGGIdentifiersToEcNumber(Task):
    ...
    config_specs = ConfigSpecs({"organism_name": StrParam(default_value="Saccharomyces cerevisiae"),
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> Table:
        ...
        for reaction in data['reactions']:
            ...
            ec_code = reaction.get('annotation', {}).get(
                'ec-code', 'EC not found')
        ...
        for ec_number, kcat in zip(ec_numbers_table.get_column_data(column_name="Ec number"),
            ...
            for info_bigg in results:
                ...
                if ec_number in info_bigg["ec_code"]:
                    final_table[ec_number] = {
                        "bigg_id": info_bigg["bigg_id"], "median Kcat": kcat}
